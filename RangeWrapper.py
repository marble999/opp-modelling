...
from RangeNet import RangeActionNet

class RangeWrapper(_NetWrapperBase):

    def __init__(self, owner, env_bldr, avrg_training_args, device):
        super().__init__(
            net=RangeActionNet(range_net_args=avrg_training_args.avrg_net_args, env_bldr=env_bldr, device=device),
            env_bldr=env_bldr,
    ...
    def get_a_probs(self, pub_obses, range_idxs, legal_actions_lists, hand_info):
        """
        Args:
            ...
            hand_info (list):           sublist of np arrays of shape [np.arr([history_len, n_features]), ...)
        """
        with torch.no_grad():
            ...
            return self.get_a_probs2(pub_obses=pub_obses, range_idxs=range_idxs, legal_action_masks=masks, hand_info=hand_info)

    def get_a_probs2(self, pub_obses, range_idxs, legal_action_masks, hand_info):
        with torch.no_grad():
